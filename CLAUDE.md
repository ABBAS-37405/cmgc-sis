...
- `supabase_fix_pay_employment.sql` — the third employment type, `Fix Pay`. Two check constraints and nothing else: no column, no table, no policy. Only needed on a database that already ran the payroll migration; `supabase_staff_payroll.sql` now carries all three values itself.
...
- `supabase_payout_accounts.sql` — `payment_method`/`bank_name`/`account_title`/`account_number` on **both** `teachers` and `staff`, so the office can check where a salary is sent before an online transfer. Four plain text columns, no table, no constraint, no policy — both tables already expose every column and already let an admin update. `PAYMENT_METHODS` and `payoutAccountLine()` in `src/lib/payroll.js` are the vocabulary and the one-line rendering; the account is shown on the Teachers/Admin Staff cards and the `StaffPayroll` salary card, printed on the payslip PDF, and **kept out of the WhatsApp slip** because that gets forwarded. The four fields also travel through `createTeacherLogin()` / `POST /api/teacher/create`, since a teacher without a login is written by the server.
- `supabase_fee_late_fine.sql` — `fees.fine_amount`, one plain numeric column, default 0, no constraint and no policy (the table already lets an admin update). See "Late-payment fine" below.
...
**Undoing a payment.** Every row in the All Transactions table has an **Undo** button (only on `Success` and `Pending Verification` rows — nothing to undo on a rejected one). It is for a payment recorded by a wrong click: `undoTransaction()` marks the transaction `Rejected` rather than deleting it (the correction is worth keeping, and every collection total on that tab already counts only `Success`), then re-derives the fee's `amount_paid` / `status` / `last_payment_date` from whatever `Success` payments remain — the same recompute as `resolve()`. It writes through `.select("id")` and treats zero rows as failure (`WRITE_BLOCKED_HINT`). A late fee already added is deliberately left alone — only "Edit Fine" on the Unpaid Fee tab clears it, same as everywhere else.

**Late-payment fine.** `fees.fine_amount` sits on top of `amount_due`, and `src/lib/lateFee.js` (imports nothing, so it is Node-drivable) is the one definition of it: `LATE_FEE_AMOUNT` (Rs 500), `LATE_FEE_STARTS_FROM` (`2026-10-01`), `isLatePayment(dueDate, paidOnISO)`, and `totalWithFine(fee)` / `feeLabelWithFine(fee)` for the combined figure everywhere one is shown. It follows the late **deposit**, not the stale due date — a fee sitting unpaid for months earns no fine on its own, exactly as the wording that asked for this put it. The two write points are `FeeVerification.jsx`'s `resolve()` (approving a proof) and `markFeePaid()` (cash recorded directly): each checks the payment's own date against the fee's `due_date`, and — only when `fine_amount` is still 0 — sets it to `LATE_FEE_AMOUNT`. Once set, nothing but the admin's own edit touches it again, so a fine already corrected by hand is never re-applied by a later partial payment. Fees due before the cutover are never fined, whenever they are finally paid — the office had not announced the rule yet. `amount_due` itself is untouched; every remaining-amount calculation, report, statement and export reads `totalWithFine()` instead, so the two numbers cannot drift apart.

**The fine is freely editable, on its own, everywhere the base "Edit Fee" control already lives.** Fee Verification's Unpaid Fee tab has an "Edit Fine" (or "Add Fine") button next to "Edit Fee" for exactly this — the office can raise it, lower it, or clear it to 0, whether it was added automatically or never applied at all. The two edits are independent: "Edit Fee" moves `amount_due` and backs the existing fine out of what is typed so the combined remaining still lands where intended; "Edit Fine" moves only `fine_amount`. Both re-derive `status` from the new combined total the same way the rest of this section already does.
...
**Three employment types on two pay shapes**, and the counts differ on purpose. `EMPLOYMENT_TYPES` holds `Regular`, `Visiting` and `Fix Pay`; the pricing knows only monthly and per-day:

- **Regular** and **Fix Pay** — a fixed `monthly_salary`, and absence is a *deduction* from it. The first leave-or-absence each month is free (`FREE_ABSENCE_DAYS`); every day after costs `monthly_salary ÷ that month's working days`. **The two price identically, line for line** — Fix Pay records the terms someone is engaged on (fixed pay rather than the regular establishment) and is a contract status, not a second salary rule.
- **Visiting** — a `per_day_salary`: `present days × per_day_salary`. There is no deduction, because nothing was owed for a day not worked. The college's word, and the same arrangement as a daily-wage sweeper.

**Nothing may branch on the type where it means the shape.** `isPerDayType()` is the single definition of that mapping, and every screen goes through it — a `=== "Visiting"` test reads correctly today and starts pricing a fourth type wrongly the moment one is added. `employmentTypeOf()` defaults anything unrecognised to Regular (a monthly salary the office can see and correct beats a null per-day rate that quietly pays nothing), and `employmentTypeSlug()` exists because `"Fix Pay".toLowerCase()` in a template className produces two classes, not one. The three strings are written down in exactly three places — `EMPLOYMENT_TYPES`, the check constraints, and the allow-list in `server.js`'s `/api/teacher/create` — and all three must agree.
