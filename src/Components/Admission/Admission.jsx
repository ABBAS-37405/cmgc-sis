...
const DOCUMENTS = [
  ...
  "B-Form",
  "Father's ID Card (NIC)",
  "Matric Marksheet",
  "NOC from Board (Optional)",
];
    ...
    return (
      <section id="admission" className="admission">
        <div className="admission__success">
          ...
          <p>Thank you, {form.name}. Your application for {form.program} has been received.</p>
          <WorkingOnIt text="Real-time saving to live database — pending Supabase connection" />
        </div>
      </section>
    );
  }
        ...
        <div className="admission__form">
          ...
          <div className="admission__field">
            <select value={form.program} onChange={update("program")} className={cls("program")}>
              <option value="">Admission Applied for Program</option>
          ...
          <div className="admission__upload">
            ...
            <div className="admission__upload-list">
              {DOCUMENTS.map((doc) => (
                <div key={doc} className="admission__upload-item">
                  <span>{doc}</span>
                  <button type="button" className="admission__upload-btn">Choose File</button>
                </div>
              ))}
            </div>
            <WorkingOnIt text="File upload — enabled once Supabase Storage is connected" />
          </div>

          <button className="admission__submit" onClick={handleSubmit}>Submit Application</button>
        </div>
      </div>
    </section>
  );
